            ...
            .run_ws_loop(
                ...
                || {
                    ...
                    async move {
                        ...
                        debug!(stream = stream_name, payload = %subscribe, "subscribing to market websocket topic");
                        ...
                        while let Some(message) = ws_stream.next().await {
                            ...
                            match message {
                                WsMessage::Text(text) => {
                                    ...
                                    if events.is_empty() {
                                        debug!(stream = stream_name, frame = %text, "ignored market websocket frame");
                                    }
                                    for event in events {
                                ...
                                WsMessage::Close(frame) => {
                                    warn!(stream = stream_name, ?frame, "market websocket closed by peer");
                                    break;
                                }
                                other => {
                                    debug!(stream = stream_name, message = ?other, "ignored non-text market websocket message");
                                }
                            }
                        }
            ...
            .run_ws_loop(
                ...
                || {
                    ...
                    async move {
                        ...
                        debug!(payload = %start, "starting private websocket stream");
                        ws_stream.send(WsMessage::Text(start.into())).await?;

                        let listen_key = loop {
                            let Some(message) = ws_stream.next().await else {
                                bail!("hibachi private websocket closed before stream.start response");
                            };
                            match message? {
                                WsMessage::Text(text) => {
                                    if let Some(listen_key) = extract_listen_key(&text) {
                                        debug!(listen_key = %listen_key, "private websocket stream started");
                                        break listen_key;
                                    }
                                    ...
                                    debug!(frame = %text, "ignored private websocket startup frame");
                                }
                                WsMessage::Ping(payload) => {
                                    ws_stream.send(WsMessage::Pong(payload)).await?;
                                }
                                WsMessage::Close(frame) => {
                                    warn!(?frame, "private websocket closed by peer during startup");
                                    bail!("hibachi private websocket closed during startup");
                                }
                                other => {
                                    debug!(message = ?other, "ignored non-text private websocket startup message");
                                }
                            }
                        };
                                ...
                                maybe_message = ws_stream.next() => {
                                    ...
                                    match message? {
                                        WsMessage::Text(text) => {
                                            let events = Self::parse_private_events(&text);
                                            if events.is_empty() {
                                                debug!(frame = %text, "ignored private websocket frame");
                                                continue;
                                            }
                                            for event in events {
                                                sender.send(event).await?;
                                        ...
                                        other => {
                                            debug!(message = ?other, "ignored non-text private websocket message");
                                        }
                                    }
                                }
                            }
