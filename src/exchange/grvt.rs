...
impl GrvtClient {
    ...
    pub async fn submit_limit_close_orders(&self, positions: &[Position]) -> Result<()> {
                        ...
                        loop {
                            tokio::select! {
                                ...
                                message = ws_stream.next() => {
                                    ...
                                    match msg? {
                                        WsMessage::Text(text) => {
                                            let raw_seen = raw_frame_count.fetch_add(1, Ordering::Relaxed);
                                            if raw_seen < 3 {
                                                debug!(
                                                    stream = stream_name,
                                                    summary = %summarize_grvt_ws_frame(&text),
                                                    "grvt market websocket frame received"
                                                );
                                            }
    ...
    async fn build_grvt_order_payload(
        ...
        info!(
            ...
            quantized_qty = %quantized_size,
            min_size = %instrument.min_size,
            base_decimals = instrument.base_decimals,
            contract_size,
            requested_price = ?request.price.map(|value| value.to_string()),
            quantized_price = ?if request.order_type == OrderType::Market {
                None
            } else {
                Some(limit_price.to_string())
            },
            tick_size = ?instrument.meta.tick_size,
            "grvt normalized order payload"
        );
...
fn summarize_grvt_ws_frame(text: &str) -> String {
    let Ok(value) = serde_json::from_str::<Value>(text) else {
        return format!("non-json len={}", text.len());
    };
    ...
    let selector = value
        ...
        .or_else(|| {
            value
                .get("result")
                .and_then(|result| result.get("subs"))
                .and_then(Value::as_array)
    ...
    if let Some(feed) = value.get("feed") {
        ...
        let price = feed
            .get("price")
            .and_then(Value::as_str)
            .or_else(|| feed.get("mid_price").and_then(Value::as_str))
            .or_else(|| feed.get("mark_price").and_then(Value::as_str))
            .unwrap_or("-");
        return format!(
            "stream={stream} selector={selector} seq={sequence} price={price} bids={bid_levels} asks={ask_levels}"
        );
    }

    format!(
        "stream={stream} selector={selector} seq={sequence} len={}",
        text.len()
    )
}
